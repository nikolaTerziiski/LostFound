...
def notify_all_users(listing: "Listing") -> int:
    ...
    print(emails)
    link = url_for("listings.detail", listing_id=listing.id)
    ...
    html = f"""
        ...
        """
    msg = Message(subject=subject, sender=current_app.config["MAIL_SENDER"], recipients=emails, body=text, html=html)
    mail.send(msg)
